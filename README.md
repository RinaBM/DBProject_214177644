...
לפני הרצת המערכת יש לפתוח את Docker Desktop ולוודא שהוא פועל. לאחר מכן יש להוריד את הפרויקט מה־GitHub כקובץ ZIP או לשכפל את הריפוזיטורי, לחלץ את הקבצים ולפתוח טרמינל בתוך תיקיית שלב ה׳:

```bash
cd "DBProject_214177644\שלב ה"
```

יש לוודא שקיים ופועל קונטיינר PostgreSQL בשם `PostgreSQL_DB`. אם הקונטיינר לא קיים, ניתן ליצור אותו בעזרת הפקודה:
...
לאחר מכן יש לשחזר את קובץ הגיבוי `backup5.backup`:
...
להרצת המערכת יש להריץ מתוך תיקיית שלב ה׳:

```bash
docker compose -p smartroute up --build
```

לאחר שהמערכת עולה, יש לפתוח בדפדפן:

```text
http://localhost:3000
```

בסיום העבודה ניתן לעצור את המערכת בעזרת `Ctrl + C`, ולאחר מכן להריץ:

```bash
docker compose down
```

הערה: אם הקונטיינר `PostgreSQL_DB` כבר קיים ופועל, אין צורך ליצור אותו מחדש. מספיק לשחזר את הגיבוי ולהריץ את האתר.
